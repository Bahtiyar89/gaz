...
function StatusGrid() {
  return <Grid container spacing={0} sx={{maxWidth: '18rem',}}>
    {[
      ...
      <Grid item xs={4}
        ...
        <Stack spacing={'1rem'} justifyContent={"center"} alignItems={'center'}>
          <Box key={i} sx={{
            ...
            {/*@ts-ignore*/}
            <CircularProgress color={item.color} variant="determinate" value={item.percentage}/>
          ...
          <Typography
            textAlign={'center'}
            variant="subtitle1" lineHeight={1.25}
          >
            {item.title}
          </Typography>
        </Stack>
...
function InputGrid() {
  return <Grid container spacing={1} sx={{maxWidth: '20rem',}}>
    {[
      'Подрядные работы',
      'Прочее',
      'Материалы',
      ...
      <Grid item xs={6}
        ...
        <TextField key={i} defaultValue={42} label={item} variant="standard" />
      </Grid>
    )}
  </Grid>
}
...
export default function MapContainer() {
  const [popupInfo, setPopupInfo] = useState<null | { lat: number; lng: number }>(null);
  const theme = useTheme()

  return <>
    <Stack visibility={popupInfo ? 'visible' : "hidden"} position={'absolute'} top={'5rem'} left={'1rem'} zIndex={'999'}>
      <Paper>
    ...
    <Stack visibility={popupInfo ? 'visible' : "hidden"} sx={{opacity: '0.7'}} position={'absolute'} bottom={'1rem'} right={'1rem'} zIndex={'999'}>
      <Paper>
        <Stack spacing={'1rem'} p={'1rem'}>
    ...
    <Stack visibility={popupInfo ? 'visible' : "hidden"} position={'absolute'} top={'5rem'} right={'1rem'} zIndex={'999'} spacing={'1rem'}>
      <Paper>
        <Stack spacing={'1rem'} p={'1rem'}>
          <InputGrid/>
        </Stack>
      ...
      <Paper>
        <Stack spacing={'1rem'} p={'1rem'}>
          <Stack>
            ...
            <Typography color={theme.palette.secondary.dark} variant={"subtitle1"}>на 01.01.2023</Typography>
          </Stack>

          <Stack>
            <Typography variant={"subtitle1"}>Управленческий учет:</Typography>
          </Stack>
          ...
          <Stack>
            <Typography variant={"subtitle1"}>Сравнительный анализ:</Typography>
          </Stack>
    ...
    {popupInfo && (
      ...
      >
        <div className={`leading-tight flex flex-col space-y-1`}>
          <p className={`text-lg leading-none`}>««Газоснабжение Сыктывкарского промузла»»</p>
          <div>
