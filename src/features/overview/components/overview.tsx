...
export function OverviewPage() {
  ...
  const {data, error, isLoading} = useSWR<IOverview>(endpoint, getData)
  ...
  if (data) return <OverviewContext.Provider value={state}>
    <Stack m={'3rem'} spacing={'1.5rem'}>
      <Heading title={"Обзор"} endpoint={endpoint} context={OverviewContext}/>
