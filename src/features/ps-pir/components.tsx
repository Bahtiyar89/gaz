...
import { getPspir } from "~/common/utils/rest-utils";
...
export function PsPirPage() {
  ...
  const { data, error, isLoading } = useSWR<any>(endpoint, getPspir);
  ...
  return (
    <PsPirContext.Provider value={state}>
      <Stack p={"3rem"} spacing={"1.5rem"}>
        <Heading
          title={"ПС ПИР"}
          endpoint={endpoint}
          context={PsPirContext}
          status={
            <CustomChip
              variants={[
                "завершено",
