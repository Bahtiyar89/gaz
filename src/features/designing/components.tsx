...
import { getDesigning } from "~/common/utils/rest-utils";
...
function DesigningEntryDate(props: {
  ...
  const plannedLength = dayjs(props.plannedDate).diff(
    dayjs(props.startingDate)
  );
...
export function DesigningPage() {
  ...
  const { data, error, isLoading } = useSWR<any>(endpoint, getDesigning);
  ...
  return (
    <DesigningContext.Provider value={state}>
      <Stack p={"3rem"} spacing={"1.5rem"}>
        <Heading
          title={"Проектирование"}
          endpoint={endpoint}
          context={DesigningContext}
          status={
            <CustomChip
              variants={[
                "завершено",
          ...
          <Stack p={"1rem"} spacing={"1rem"}>
            <DesigningEntryDate
              ...
              onPlannedChange={(date) => {
                if (date)
                  state!.designAssignment.plannedDate = dateFormat(date);
              }}
            ...
            <DesigningEntryDate
              ...
              onPlannedChange={(date) => {
                if (date)
                  state!.technicalSolutions.plannedDate = dateFormat(date);
              }}
              actualDate={snap.technicalSolutions.actualDate}
              onActualChange={(date) => {
                if (date)
                  state!.technicalSolutions.actualDate = dateFormat(date);
              }}
            />
            ...
            <DesigningEntryDate
              ...
              onPlannedChange={(date) => {
                if (date)
                  state!.territoryLayoutDocs.plannedDate = dateFormat(date);
              }}
              actualDate={snap.territoryLayoutDocs.actualDate}
              onActualChange={(date) => {
                if (date)
                  state!.territoryLayoutDocs.actualDate = dateFormat(date);
              }}
            />
            ...
            <DesigningEntryDate
              ...
              onPlannedChange={(date) => {
                if (date)
                  state!.projectDocsApproval.plannedDate = dateFormat(date);
              }}
              actualDate={snap.projectDocsApproval.actualDate}
              onActualChange={(date) => {
                if (date)
                  state!.projectDocsApproval.actualDate = dateFormat(date);
              }}
            />
            ...
            <DesigningEntryDate
              ...
              onPlannedChange={(date) => {
                if (date)
                  state!.departmentalExpertise.plannedDate = dateFormat(date);
              }}
              actualDate={snap.departmentalExpertise.actualDate}
              onActualChange={(date) => {
                if (date)
                  state!.departmentalExpertise.actualDate = dateFormat(date);
              }}
            ...
            <DesigningEntryDate
              ...
              onPlannedChange={(date) => {
                if (date)
                  state!.stateEnvironmentalExpertise.plannedDate =
                    dateFormat(date);
              }}
              actualDate={snap.stateEnvironmentalExpertise.actualDate}
              onActualChange={(date) => {
                if (date)
                  state!.stateEnvironmentalExpertise.actualDate =
                    dateFormat(date);
              }}
              status={<Chip label="ок" color="success" />}
            />
            ...
            <DesigningEntryDate
              ...
              onPlannedChange={(date) => {
                if (date)
                  state!.mainStateExpertise.plannedDate = dateFormat(date);
              }}
              actualDate={snap.mainStateExpertise.actualDate}
              onActualChange={(date) => {
                if (date)
                  state!.mainStateExpertise.actualDate = dateFormat(date);
              }}
              status={<Chip label="ок" color="success" />}
            />
            ...
            <DesigningEntryDone
              ...
              onPlannedChange={(event) => {
                state!.landManagement.plannedDone = parseFloat(
                  event.target.value
                );
              }}
              actualDone={snap.landManagement.actualDone}
              onActualChange={(event) => {
                state!.landManagement.actualDone = parseFloat(
                  event.target.value
                );
              }}
              status={<Chip label="ок" color="success" />}
            ...
            <DesigningEntryDone
              ...
              onPlannedChange={(event) => {
                state!.facilityEquipment.plannedDone = parseFloat(
                  event.target.value
                );
              }}
              actualDone={snap.facilityEquipment.actualDone}
              onActualChange={(event) => {
                state!.facilityEquipment.actualDone = parseFloat(
                  event.target.value
                );
              }}
              status={<Chip label="ок" color="success" />}
            ...
            <DesigningEntryDone
              ...
              onPlannedChange={(event) => {
                state!.customerDelivery.plannedDone = parseFloat(
                  event.target.value
                );
              }}
              actualDone={snap.customerDelivery.actualDone}
              onActualChange={(event) => {
                state!.customerDelivery.actualDone = parseFloat(
                  event.target.value
                );
              }}
              status={<Chip label="ок" color="success" />}
            ...
            <DesigningEntryDone
              ...
              onPlannedChange={(event) => {
                state!.construction.plannedDone = parseFloat(
                  event.target.value
                );
              }}
            ...
            <DesigningEntryDone
              ...
              onPlannedChange={(event) => {
                state!.contractorDelivery.plannedDone = parseFloat(
                  event.target.value
                );
              }}
              actualDone={snap.contractorDelivery.actualDone}
              onActualChange={(event) => {
                state!.contractorDelivery.actualDone = parseFloat(
                  event.target.value
                );
              }}
              status={<Chip label="ок" color="success" />}
