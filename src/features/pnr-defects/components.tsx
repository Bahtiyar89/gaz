...
function PnrDefectsEntries() {
  ...
  const cols: GridColDef[] = [
    {field: 'orderDate', headerName: 'Дата предписания', width: 200, editable: true},
    {field: 'orderNumber', headerName: 'Номер предписания', width: 200, editable: true},
    {
      field: 'defectType',
    ...
    {field: 'fixPlanned', headerName: 'Срок устранения план', width: 200, editable: true},
    {field: 'fixActual', headerName: 'Срок устранения факт', width: 200, editable: true},
    {field: 'defectDescription', headerName: 'Выявленные несоответствия', width: 200, editable: true},
    {field: 'annotations', headerName: 'Примечания', width: 200, editable: true},
  ];
...
export function PnrDefectsPage() {
  ...
  const {data, error, isLoading} = useSWR<IPnrDefects>(endpoint, getData)
  ...
  return <PnrDefectsContext.Provider value={state}>
    <Stack p={'3rem'} spacing={'1.5rem'}>
      <Heading title={'ПНР: Дефекты'} endpoint={endpoint} context={PnrDefectsContext}
               status={<CustomChip
